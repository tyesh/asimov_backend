...
async function isUserLogged(driver, env="local", res) {
    ...
    if(currentURL == LOGIN_URL){
        res.write(printMessage("Error", "Credenciales de usuario incorrectas", ERROR));
        return false;
    }
    return true;
}
...
async function login(driver, env="local", user, password, res) {
    res.write(printMessage("Ingreso", "Ingresando a la vista de inicio de sesión"));
    let envLoginURL = (env === "testing" ? BASE_URL_TESTING : BASE_URL_LOCAL) + LOGIN_URL;
...
async function getAccessToken(driver, env="local", res) {
    ...
    try {
        let envHomeURL = (env === "testing" ? BASE_URL_TESTING : BASE_URL_LOCAL) + BASE_URL_HOME;
        var currentURL = await driver.getCurrentUrl();
        if(currentURL != envHomeURL){
            res.write(printMessage("Error", "Credenciales de usuario incorrectas", ERROR));
            return;
        }
        await driver.executeScript("return window.localStorage.getItem('" + ACCESS_TOKEN + "');");
...
module.exports.doLogin = async function makeLogin(driver, user, password, env = "local", res) {
    await sleep();
